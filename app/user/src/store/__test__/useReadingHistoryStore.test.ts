...
import {
  useApiBookReadingProgress as realUseApiBookReadingProgress,
  useApiBooks as realUseApiBooks,
} from '@guga-reading/shares';
import { useMessage } from 'qyani-components';
import { useReadingHistoryStore } from '../useReadingHistoryStore';

const useApiBookReadingProgress = realUseApiBookReadingProgress as unknown as {
  get: Mock;
  update: Mock;
  delete: Mock;
};
const useApiBooks = realUseApiBooks as unknown as {
  getBookBySelect: Mock;
  getBookById: Mock;
  getCatalogById: Mock;
  getBooksByList: Mock;
  getBookCategory: Mock;
};
